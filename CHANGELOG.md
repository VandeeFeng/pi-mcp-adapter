...
### Changed
- Reduced MCP failed-connection backoff from 60 seconds to 30 seconds.
- Improved `/mcp` panel reconnect feedback: retrying during failed backoff now shows an inline `(retry after Ns)` notice instead of silently doing nothing.

## [0.1.30] - 2026-05-01

### Changed
- Updated TypeBox dependency from `@sinclair/typebox` to the new `typebox` package and adjusted imports for compatibility with the latest Pi update.
...
### Added
- New `settings.notify` option (default: false) to enable MCP lifecycle notifications in TUI

### Changed
- MCP lifecycle events (reconnect, idle shutdown, errors) now output to console by default
- When `settings.notify: true`, lifecycle events display as TUI notifications instead

## [0.1.24] - 2026-04-25

### Added
- New `settings.showStatus` option (default: false) to show/hide MCP status in bottom bar
...
### Changed
- ESC key also cancels server connection attempts when using `mcp({ connect: "server-name" })`

## [0.1.21] - 2026-04-16

### Changed
- ESC key now properly aborts MCP tool calls immediately instead of waiting for completion

## [0.1.2] - 2026-03-26

### Fixed
- MCP tool call error messages now properly fold and display with correct background styling
